...
anaSim::anaSim(Int_t z,Int_t nDer){
  ...
  TNtuple *ntuplePulse = new TNtuple("ntuplePulse","ntuplePulse","ientry:pmtNum:nhits:charge:startTime:peakWidth:T0:V0:C0:vMax:vMaxTime:Sdev:baseline");
  ...
  TNtuple *ntupleEvent = new TNtuple("ntupleEvent","ntupleEvent","irun:ientry:Sdev:baseline:integral:deltaT:tMax:vMax:cMax:totalCharge");
  ...
  for(Long64_t ientry = 0; ientry < nentries; ientry++){
    ...
    */
    ...
    for(int j = 0; j < signal.size();j++){
      ...
      hFlatBaseline[j] = new TH1D(TString("FlatBaseline")+to_string(ientry),TString("FlatBaseline")+to_string(ientry),NEvents,pmtEvent->time[0],pmtEvent->time[NEvents-1]);
    ...
    std::vector<std::vector<Double_t> > flatBaseline;
    flatBaseline.resize(1);
    
    //flatBaseline[0] = BaselineSubtraction(signal[0],100,0,ientry);
    sum = 0;
    //cout<<"finished first loop"<<endl;
    for(int j = 0; j < derivative.size(); j++){
      for(int i = 0; i < derivative[j].size(); i++){
        //signal[j][i] /= (double) nInt;
        //cout<<flatBaseline[j][i]<<endl;
        sum += signal[j][i];
        ...
        //hFlatBaseline[j]->SetBinContent(i+1,flatBaseline[j][i]);
      }
    }
    ...
    */
    ...
    for(int j = 0; j < signal.size(); j++){
      ...
      for(int i = 0; i < peakTime[j].size()-1; i += 2){
        ...
        ntuplePulse->Fill(ientry,j,peakTime[j].size()/2,-charge*deltaT,startTime,peakWidth,T0,V0,C0,-vMax,vMaxTime,0,0);
      ...
      ntupleEvent->Fill(z,ientry,Sdev,baseline,-sum,deltaT,tMaxEvent,-vMaxEvent,-cMaxEvent,-totalCharge*deltaT);//-vMaxEvent,tMaxEvent);
